...
config.PartitionManager = ConfigSubsection()
...
def parseCmd(result):
        ...
        try:
                ...
                print("[PartitionManager] <parse error>")
                return []
        return devlist
...
def myExecute(cmd, session, test=False):
        if test:
                ...
                res = os_system(cmd)
                result = (res >> 8)
        print("[PartitionManager]", result, cmd)
...
class PartitionManager(Screen):
        screenwidth = getDesktop(0).size().width()
        if screenwidth and screenwidth == 1920:
                skin = """<screen position="center,center" size="700,300" title="Partition Manager">
...
class AddPart(Screen, ConfigListScreen):
        ...
        def __init__(self, session, maxsize, unit, countpart):
                ...
                if pathExists("/usr/sbin/mkfs.msdos"):
                        menu.append("fat16")
                config.PartitionManager.fs = NoSave(ConfigSelection(default=default, choices=menu))
                config.PartitionManager.size = NoSave(ConfigInteger(default=maxsize, limits=[1, maxsize]))
                list = []
                if countpart < 4:#nur 4 parts möglich bei primary
                        list.append(getConfigListEntry(_("size in %s (max %d %s):") % (unit, maxsize, unit), config.PartitionManager.size))
                list.append(getConfigListEntry(_("filesystem:"), config.PartitionManager.fs))
        ...
        def keySave(self):
                if config.PartitionManager.size.value > 0:
                        self.close((config.PartitionManager.size.value, config.PartitionManager.fs.value))
...
class Cpart(Screen):
        ...
        skin = """<screen position="center,center" size="770,300" title="PartitionManager">
        ...
        def KeyRed(self):
                ...
                if sel and sel[1] and sel[5][PA_TYPE] & self.PA_TYPE_LAST and bool(sel[5][PA_TYPE] & self.PA_TYPE_FREE) == False:
                        try:
                                ...
                                print("[PartitionManager] <remove part>")
                        self.__Filllist()
        ...
        def __createCommandList(self):
                ...
                #for x in self.__comlist: print "[PartitionManager] com =",x
                if len(self.__comlist):
...
class Cpartexe(Screen):
        ...
        def __getPartitionUUID(self, device):
                try:
                        ...
                        print("[PartitionManager] <error get UUID>")
                return None
