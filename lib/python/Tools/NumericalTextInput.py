...
class NumericalTextInput:
	...
	def __init__(self, nextFunc=None, handleTimeout=True, search=False, mapping=None, mode=None):
		self.nextFunction = nextFunc if callable(nextFunc) else None
		if handleTimeout:
			self.timer = eTimer()
		...
		# print(f"[NumericalTextInput] DEBUG: Mode='{mode}', Index={index}, Character set: '{''.join(sorted(self.useableChars))}'")
		self.lastKey = -1
		self.pos = -1
