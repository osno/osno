...
def InitUsageConfig():
	...
	config.usage.menuEntryStyle = ConfigSelection(default="both", choices=[
...
#	config.usage.rc_model = ConfigSelection(default=DefaultRemote, choices=RemoteChoices)
	...
	config.usage.date.dayfull = ConfigSelection(default=_("%A %d.%m.%Y") if config.misc.locale.value == "it_IT" else _("%A %-d %B %Y"), choices=choicelist)
