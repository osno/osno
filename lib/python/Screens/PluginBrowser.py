...
PLUGIN_CATEGORIES = {
	...
	"security": _("Security"),
	"settings": _("Setting"),
	"skincomponents": _("Skin Component"),
	"skinpacks": _("Plugin Skin"),
	"skins": _("Skin"),
	"softcams": _("Softcam"),
	"src": _("Source"),
	"systemplugins": _("System Plugin"),
	"vix": _("OpenViX"),
	"weblinks": _("Web Link")
}
PACKAGE_CATEGORIES = {
	"": _("Other"),
	"base": _("Base"),
	# "base/utils": _("Base Utility"),
	"base/shell": _("Base Shell"),
	"console": _("Console"),
	# "console/admin": _("Console Administrative"),  # Now bundled into the "Other" category.
	# "console/multimedia": _("Console Multimedia"),  # Now bundled into the "Other" category.
	"console/network": _("Console Network"),
	"console/utils": _("Console Utility"),
	"devel": _("Development"),
	"devel/python": _("Python Development"),
	# "doc": _("Documentation"),  # Now bundled into the "Other" category.
	# "enigma2": _("Enigma2s"),  # Now bundled into the "Other" category.
	# "font": _("Font"),  # Now bundled into the "Other Packages" category.
	"kernel": _("Kernel"),
	"libs": _("Library"),
	"multimedia": _("Multimedia"),
	"network": _("Network"),
	"plugin": _("Plugin"),
	"universe/otherosfs": _("File System"),
	"utils": _("Utility"),
	"x11": _("X11")
}
...
config.usage.pluginListLayout = ConfigSelection(default=PLUGIN_LIST, choices=[
...
config.pluginfilter.extraopkg = ConfigYesNo(default=False)
...
class PluginBrowser(Screen, HelpableScreen, NumericalTextInput, ProtectedScreen):
	...
	def __init__(self, session):
		self.layout = "pluginList" if config.usage.pluginListLayout.value == PLUGIN_LIST else "PluginBrowser"
		...
		self.skinName = ["PluginBrowserList" if config.usage.pluginListLayout.value == PLUGIN_LIST else "PluginBrowser"]
	...
	def createGUIScreen(self, parent, desktop, updateonly=False):
		for item in self.renderer:
			if isinstance(item, GUIComponent) and item.__class__.__name__ == "Listbox":
...
class PackageAction(Screen, HelpableScreen, NumericalTextInput):
	...
	def __init__(self, session, mode=MODE_REMOVE):
		...
		if mode <= self.MODE_MANAGE:
			...
			if not config.pluginfilter.extraopkg.value:
